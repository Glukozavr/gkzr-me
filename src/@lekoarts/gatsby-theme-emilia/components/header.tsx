...
const Header = () => {
    ...
    const logo = useStaticQuery<LogoStaticQuery>(graphql`
    query {
      file(name: { eq: "logo" }) {
        childImageSharp {
          gatsbyImageData(layout: FIXED, width: 367, height: 200, quality: 100)
        }
      }
    }
  `)
        ...
        <Flex as="header" variant="layout.projectHead">
            ...
            <div sx={{ textAlign: `center`, my: 5, zIndex: 10 }}>
                <animated.div style={fadeProps}>
                    <div>
                        {logo?.file?.childImageSharp?.gatsbyImageData ? (
                            <GatsbyImage image={logo.file.childImageSharp.gatsbyImageData} alt="Logo" />
                        ) : (
                            <div
                                sx={{
                                    fontSize: 0,
                            ...
                            >
                                Place an image with the name "Logo" inside the directory "{assetsPath}"
                            </div>
                        )}
                    </div>
                </animated.div>
